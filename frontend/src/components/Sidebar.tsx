...
const Sidebar: React.FC<SidebarProps> = ({ mobileOpen, onMobileClose }) => {
  ...
  const [open, setOpen] = useState<Record<string, boolean>>({ intro: true, catalog: true });
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 h-full w-64 bg-[#0b1120] flex flex-col z-40 transition-transform duration-300
          ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}
      >
        {/* Logo */}
        <div className="flex items-center justify-between px-5 py-5 border-b border-slate-700/50 flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="bg-teal-600 rounded-lg p-1.5 flex-shrink-0">
              <Pill className="h-5 w-5 text-white" />
            </div>
            <div>
              <p className="text-white font-bold text-[15px] tracking-tight leading-none">PharmaCare</p>
