...
{
    Write-Host "Setting database connection for $($dataSetId)"
...
Write-Host "Starting PowerBI Artifact Provisioning"
...
foreach ($powerBIReport in $reportList) {
    ...
    Write-Host "Uploading $($powerBIReport.Name) Report"
...
Write-Host "Create PowerBI linked service $($keyVaultName)"
$powerBIDataSetConnectionTemplate = Get-Content -Path "$templatesPath/powerbi_dataset_connection.json"
...
Write-Host "Setting PowerBI Report Data Connections" 
...
foreach ($powerBIReport in $reportList) {
        ...
        {
                Write-Host "Setting database connection for $($powerBIReport.Name)"
