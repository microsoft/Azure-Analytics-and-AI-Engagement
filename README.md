...
## What is it?
Deployable PoC Accelerators (DPoC) are packaged Industry Scenario Deployable Demos with ARM templates (with a demo web app, Power BI reports, Synapse resources, AML Notebooks etc.) that can be deployed in a customer’s subscription using the CAPE tool in a few hours.  Partners can also deploy DeployableEAM Demos in their own subscriptions using DPoC.
