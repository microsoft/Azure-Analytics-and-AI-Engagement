...
import { apiRoutes, pharmaApi } from '../services/apiClient';

const API_BASE = apiRoutes.pharma.medications;
...
>(
  ...
  async (params, { getState }: { getState: () => unknown }) => {
    ...
    try {
      const res = await pharmaApi.get(`${API_BASE}?${qs}`);
...
export const createMedication = createAsyncThunk<Medication, Omit<Medication, 'id' | 'inStock' | 'featured'>>(
  ...
  async (data) => {
    const res = await pharmaApi.post(API_BASE, data);
...
export const updateMedication = createAsyncThunk<Medication, Medication>(
  ...
  async (data) => {
    await pharmaApi.put(`${API_BASE}/${data.id}`, { ...data, id: Number(data.id) });
...
export const deleteMedication = createAsyncThunk<string, string>(
  ...
  async (id) => {
    await pharmaApi.delete(`${API_BASE}/${id}`);
    return id;
  }
);
