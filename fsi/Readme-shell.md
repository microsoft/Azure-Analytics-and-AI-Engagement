## What is a DPoC?
Deployable PoC Accelerators (DPoC) are packaged Demos using ARM templates and automation scripts (with a demo web application, Power BI reports, Fabric resources, ML Notebooks, etc.) that can be deployed in a customer’s environment.
...
* An eligible **Microsoft 365** license is required per user to access the agent in **Microsoft 365 Copilot**
...
* Make sure the **Fabric Administrator** role is assigned to your Entra ID by the **Global Administrator**.
* Before running the deployment script, a **Fabric Administrator** must **enable these tenant settings** in the **Fabric Admin Portal** (Settings, Admin portal, Tenant settings):
  - Users can use **Copilot, AI Agents and other AI experiences powered by Azure OpenAI**, under Copilot and AI. This also covers the Fabric data agent the script creates.
  ...
  - D**ata sent to Azure OpenAI can be stored outside your capacity's geographic region, compliance boundary, or national cloud instance**, under Copilot and AI. Enabling this accepts the preview terms.
  - Allow up to 15 minutes for tenant setting changes to take effect.
...
* Select a region where the desired Azure Services are available. If certain services are not available, deployment may fail. See [Azure Services Global Availability](https://azure.microsoft.com/en-us/global-infrastructure/services/?products=all) for understanding target service availability (consider the region availability for App Service, Azure OpenAI models, AI Search, Microsoft Fabric, and cognitive services while choosing a location).
...
* Once the resources have been set up, ensure that your AD user have the “Storage Blob Data Owner” role assigned on the storage account name starting with “storage”.
...
>**Note:** In this task, we will execute a PowerShell script on CloudShell to deploy assets which will approximately take 40 minutes.
...
8. Enter you **PAT token** for login and press **Enter** key.

    ![Git Clone Command to Pull Down the demo Repository.](media/cloudshell2.png)
...
10. **Press** **Y** and click on the **Enter** button.
      
    ![Yes.](media/yes.png)

11. From the Azure Cloud Shell, **copy** the authentication code. You will need to enter the code in the next step.
...
13. **Paste** the authentication code and click on the **Next** button.

    ![box](media/cloud-shell-7.png) 

14. **Select** the user account you used for logging into the Azure Portal in [Task 1](#task-1-fabric-workspace-creation).
...
18. **Enter** the number on the left side of your subscription name from the screen and press **Enter** key.
...
19. **Copy** the code on the screen to authenticate the **Azure PowerShell script** for creating reports in Power BI. **Click** the link [https://microsoft.com/devicelogin](https://microsoft.com/devicelogin).

    ![Authentication link and Device code.](media/cloud-shell-10.png)

20. A new browser window will launch. **Paste** the **authentication code** you copied from the shell above and press **Enter**.

    ![box](media/cloud-shell-11.png) 

21. **Select** the user account that is used for logging into the Azure Portal in [Task 1](#task-1-fabric-workspace-creation).
...
23. **Close** the browser tab when you see the message box.

    ![box](media/cloud-shell-13.png) 

24. Go back to the **Azure Cloud Shell** execution window.

25. **Click** on the URL [https://microsoft.com/devicelogin](https://microsoft.com/devicelogin).

    ![Click the link.](media/cloud-shell-100.png)
      
26. In the new browser tab, **paste** the code you copied and **click** on **Next**.
...
27. Select the **user account** you used to log into the **Azure Portal** in [Task 1](#task-1-fabric-workspace-creation). 
...
29. **Close** the browser tab when you see the message box.

    ![box](media/cloud-shell-104.png) 

30. **Enter** the Region for deployment with the necessary resources available, preferably "eastus2".
...
31. **Enter** the Workspace ID that you copied in [Task 1](#task-1-fabric-workspace-creation) consecutively.
...
32. A screen similar to the screenshot below indicates the end of your script execution.
